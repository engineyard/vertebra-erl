...
run(Config, Op, Inputs, Token, Target, ClientRef) when is_list(Inputs),
  ...
  {ok, CP} = vertebra_command:start_link(Config, Op, R1, Target, Token),
  unlink(CP),
  vertebra_command:execute(CP, ClientRef);

run(Op, Inputs, Token, Target, ClientRef, XMPP) when is_list(Inputs),
                                                     is_tuple(ClientRef),
                                                     is_pid(XMPP) ->
  {ok, R1} = xml_util:convert(to, Inputs),
  {ok, CP} = vertebra_command:start_link([], Op, R1, Target, Token, XMPP),
  vertebra_command:execute(CP, ClientRef).
...
iq(_XMPP, _Type, From, _QueryNS, PacketId, Attrs, Elts, State) ->
  case vertebra_xmpp:get_token(Elts) of
    undefined ->
      State;
    _ ->
        ...
        CommandPid ->
          ...
          case FnName of
            undefined ->
              State;
            _ ->
              NextPendingAction= vertebra_command:FnName(CommandPid, From, PacketId, Attrs, Elts),
...
handle_cast({execute, ClientInfo}, State) ->
  ...
  Op = ops_builder:generic_op({State#state.command,
                               State#state.token,
                               State#state.inputs}),
  vertebra_xmpp:send_set(State#state.xmpp, State#state.targetjid, Op),
...
handle_call({complete_ack, From, PacketId, _Attrs, Elts}, _From, State) ->
  ...
  vertebra_xmpp:send_result(State#state.xmpp, From, PacketId, Ack),
  {reply, ok, State};

handle_call({receive_op_results, From, PacketId, _Attrs, Elts}, _From, State) ->
  [H|_T] = Elts,
  vertebra_xmpp:send_result(State#state.xmpp, From, PacketId, ops_builder:final_op(State#state.token)),
...
test() ->
  ...
  vertebra_command:run(Config, Op, Resources, Token, Target).
